<template>
  <LayoutPenjual :statusToko="statusToko" :tokoId="tokoId">
    <div class="page-header">
      <div class="container">
        <h1 class="page-title">Profil Toko</h1>
        <p class="page-sub">
          {{
            mode === 'edit'
              ? 'Edit informasi toko kamu'
              : 'Informasi toko yang dilihat pembeli'
          }}
        </p>
      </div>
    </div>

    <div class="container main-content">
      <div v-if="loadingData" class="loading-wrap">
        <div class="spinner"></div>
        <p>Memuat data toko...</p>
      </div>

      <div v-else class="form-layout">
        <!-- STATUS BANNER — read only, tidak bisa diubah penjual -->
        <div class="status-banner" :class="statusClass">
          <span class="status-icon-sm">{{ statusIcon }}</span>
          <div>
            <p class="status-label">Status Toko</p>
            <p class="status-val">{{ statusText }}</p>
            <p v-if="toko.alasan_tolak" class="status-alasan">
              Alasan: "{{ toko.alasan_tolak }}"
            </p>
          </div>
        </div>

        <div v-if="errorMsg" class="alert-error">⚠️ {{ errorMsg }}</div>
        <div v-if="successMsg" class="alert-success">✅ {{ successMsg }}</div>

        <!-- FOTO TOKO -->
        <div class="form-section">
          <h2 class="section-title">Foto Toko</h2>
          <div class="foto-layout">
            <div
              class="foto-preview-wrap"
              @click="mode === 'edit' ? triggerUpload() : null"
              :style="mode === 'edit' ? 'cursor:pointer' : 'cursor:default'"
            >
              <img
                v-if="previewUrl"
                :src="previewUrl"
                class="foto-preview"
                alt="foto toko"
              />
              <div v-else class="foto-placeholder">🏪</div>
              <div v-if="mode === 'edit'" class="foto-overlay">📷 Ganti</div>
            </div>
            <div class="foto-info">
              <p class="foto-hint">
                Foto toko membuat pembeli lebih percaya. Gunakan logo atau foto
                produk unggulan.
              </p>
              <button
                v-if="mode === 'edit'"
                type="button"
                class="btn-upload-foto"
                @click="triggerUpload"
              >
                Upload Foto Toko
              </button>
              <input
                ref="fileInput"
                type="file"
                accept="image/*"
                @change="onFileChange"
                class="hidden"
              />
            </div>
          </div>
        </div>

        <!-- INFO TOKO -->
        <div class="form-section">
          <h2 class="section-title">Informasi Toko</h2>

          <div class="field">
            <label>Nama Toko</label>
            <input
              v-if="mode === 'edit'"
              v-model="form.nama_toko"
              type="text"
              placeholder="Nama tokomu"
              maxlength="60"
              :disabled="loading"
            />
            <p v-else class="view-value">{{ form.nama_toko || '-' }}</p>
          </div>

          <div class="field">
            <label>Nomor WhatsApp</label>
            <div v-if="mode === 'edit'" class="input-prefix-wrap">
              <span class="prefix">+62</span>
              <input
                v-model="form.nomor_wa"
                type="tel"
                placeholder="081234567890"
                :disabled="loading"
              />
            </div>
            <p v-else class="view-value">+62{{ form.nomor_wa || '-' }}</p>
            <span v-if="mode === 'edit'" class="field-hint"
              >Nomor ini digunakan pembeli untuk menghubungi kamu</span
            >
          </div>

          <div class="field">
            <label>Alamat</label>
            <input
              v-if="mode === 'edit'"
              v-model="form.alamat"
              type="text"
              placeholder="Jl. contoh No. 5, RT 02, Desa ..."
              :disabled="loading"
            />
            <p v-else class="view-value">{{ form.alamat || '-' }}</p>
          </div>

          <div class="field">
            <label>Deskripsi <span class="optional">(opsional)</span></label>
            <textarea
              v-if="mode === 'edit'"
              v-model="form.deskripsi"
              rows="4"
              placeholder="Ceritakan tentang toko kamu..."
              maxlength="300"
              :disabled="loading"
            ></textarea>
            <p v-else class="view-value multiline">
              {{ form.deskripsi || 'Tidak ada deskripsi' }}
            </p>
            <span v-if="mode === 'edit'" class="field-hint"
              >{{ form.deskripsi.length }}/300 karakter</span
            >
          </div>
        </div>

        <!-- ACTIONS -->
        <div class="form-actions">
          <RouterLink to="/toko/dashboard" class="btn-cancel"
            >Kembali</RouterLink
          >

          <!-- Mode VIEW -->
          <button
            v-if="mode === 'view'"
            class="btn-submit"
            @click="mode = 'edit'"
          >
            ✏️ Edit Profil
          </button>

          <!-- Mode EDIT -->
          <template v-else>
            <button
              type="button"
              class="btn-cancel"
              @click="cancelEdit"
              :disabled="loading"
            >
              Batal
            </button>
            <button
              type="button"
              class="btn-submit"
              @click="handleSubmit"
              :disabled="loading || uploading"
            >
              <span v-if="loading || uploading" class="spinner-sm"></span>
              <span v-else>💾 Simpan</span>
            </button>
          </template>
        </div>
      </div>
    </div>
  </LayoutPenjual>
</template>

<script setup>
import { ref, computed, onMounted } from 'vue';
import { useRouter } from 'vue-router';
import { supabase } from '@/lib/supabase';
import LayoutPenjual from '@/layouts/LayoutPenjual.vue';

const router = useRouter();
const mode = ref('view');
const loadingData = ref(true);
const loading = ref(false);
const uploading = ref(false);
const errorMsg = ref('');
const successMsg = ref('');
const fileInput = ref(null);
const previewUrl = ref('');
const fotoFile = ref(null);
const tokoId = ref(null);
const statusToko = ref('');

// Pisahkan: data read-only (tidak boleh diedit penjual)
const toko = ref({ status: '', alasan_tolak: '' });

// Data yang boleh diedit penjual
const form = ref({
  nama_toko: '',
  nomor_wa: '',
  alamat: '',
  deskripsi: '',
  foto_toko: '',
});

// Snapshot untuk fungsi batal
let snapshot = null;

const statusClass = computed(() => ({
  'status-aktif': toko.value.status === 'AKTIF',
  'status-pending': toko.value.status === 'PENDING',
  'status-tolak': toko.value.status === 'DITOLAK',
  'status-blokir': toko.value.status === 'DIBLOKIR',
}));

const statusIcon = computed(
  () =>
    ({ AKTIF: '✅', PENDING: '⏳', DITOLAK: '❌', DIBLOKIR: '🚫' })[
      toko.value.status
    ] ?? '❓',
);

const statusText = computed(
  () =>
    ({
      AKTIF: 'Toko aktif dan bisa menerima order',
      PENDING: 'Menunggu verifikasi admin',
      DITOLAK: 'Pendaftaran ditolak admin',
      DIBLOKIR: 'Toko diblokir oleh admin',
    })[toko.value.status] ?? '-',
);

const triggerUpload = () => fileInput.value?.click();

const onFileChange = (e) => {
  const file = e.target.files[0];
  if (!file) return;
  if (file.size > 2 * 1024 * 1024) {
    errorMsg.value = 'Ukuran foto maksimal 2MB.';
    return;
  }
  if (previewUrl.value?.startsWith('blob:'))
    URL.revokeObjectURL(previewUrl.value);
  fotoFile.value = file;
  previewUrl.value = URL.createObjectURL(file);
  errorMsg.value = '';
};

const uploadFoto = async () => {
  uploading.value = true;
  const ext = fotoFile.value.name.split('.').pop();
  const nama = `toko-${tokoId.value}-${Date.now()}.${ext}`;
  const { error } = await supabase.storage
    .from('foto-toko')
    .upload(nama, fotoFile.value, { upsert: true });
  if (error) {
    uploading.value = false;
    return null;
  }
  const { data } = supabase.storage.from('foto-toko').getPublicUrl(nama);
  uploading.value = false;
  return data.publicUrl;
};

const handleSubmit = async () => {
  // Validasi manual — tidak pakai <form> agar tidak bentrok dengan toggle mode view/edit
  errorMsg.value = '';
  if (!form.value.nama_toko.trim()) {
    errorMsg.value = 'Nama toko wajib diisi.';
    return;
  }
  if (!form.value.nomor_wa.trim()) {
    errorMsg.value = 'Nomor WhatsApp wajib diisi.';
    return;
  }
  // Validasi Format Nomor WA (Hanya angka & minimal panjang)
  const waRegex = /^[0-9]+$/;
  if (!waRegex.test(form.value.nomor_wa.trim())) {
    errorMsg.value = 'Nomor WhatsApp hanya boleh berisi angka.';
    return;
  }
  if (form.value.nomor_wa.trim().length < 10) {
    errorMsg.value = 'Nomor WhatsApp minimal 10 digit.';
    return;
  }
  if (!form.value.alamat.trim()) {
    errorMsg.value = 'Alamat wajib diisi.';
    return;
  }

  loading.value = true;
  successMsg.value = '';

  let fotoUrl = previewUrl.value?.startsWith('blob:') ? null : previewUrl.value;

  if (fotoFile.value) {
    fotoUrl = await uploadFoto();
    if (!fotoUrl) {
      errorMsg.value = 'Gagal upload foto. Coba lagi.';
      loading.value = false;
      return;
    }
  }

  let nomorWaClean = form.value.nomor_wa.trim();
  if (nomorWaClean.startsWith('0')) {
    nomorWaClean = nomorWaClean.substring(1);
  }

  // HANYA update kolom yang boleh diubah penjual
  // status, alasan_tolak, user_id TIDAK ikut diupdate
  const { error } = await supabase
    .from('toko')
    .update({
      nama_toko: form.value.nama_toko.trim(),
      nomor_wa: nomorWaClean,
      alamat: form.value.alamat.trim(),
      deskripsi: form.value.deskripsi.trim(),
      ...(fotoUrl ? { foto_toko: fotoUrl } : {}),
    })
    .eq('id', tokoId.value);

  if (error) {
    errorMsg.value = 'Gagal menyimpan. Coba lagi.';
    loading.value = false;
    return;
  }

  successMsg.value = 'Profil toko berhasil diperbarui!';
  snapshot = { ...form.value }; // perbarui snapshot dengan data terbaru
  fotoFile.value = null;
  mode.value = 'view';
  await updateViewData(); // refresh data dari server untuk memastikan sinkronisasi dengan database
  loading.value = false;
};

const cancelEdit = () => {
  form.value = { ...snapshot };
  previewUrl.value = snapshot.foto_toko ?? '';
  fotoFile.value = null;
  errorMsg.value = '';
  mode.value = 'view';
};

const updateViewData = async () => {
   const {
    data: { session },
  } = await supabase.auth.getSession();
  if (!session) {
    router.push('/login');
    return;
  }

  const { data } = await supabase
    .from('toko')
    .select('*')
    .eq('user_id', session.user.id)
    .single();
  if (!data) {
    router.push('/');
    return;
  }

  statusToko.value = data.status;
  tokoId.value = data.id;

  // Pisahkan data read-only dan editable
  toko.value = {
    status: data.status ?? '',
    alasan_tolak: data.alasan_tolak ?? '',
  };

  form.value = {
    nama_toko: data.nama_toko ?? '',
    nomor_wa: data.nomor_wa ?? '',
    alamat: data.alamat ?? '',
    deskripsi: data.deskripsi ?? '',
    foto_toko: data.foto_toko ?? '',
  };

  snapshot = { ...form.value };
  previewUrl.value = data.foto_toko ?? '';
};

onMounted(async () => {
  await updateViewData();
  loadingData.value = false;
});
</script>

<style scoped>
.page-header {
  background: linear-gradient(135deg, #2d5016, #3a6b1e);
  padding: 2.5rem 0;
}
.container {
  max-width: 680px;
  margin: 0 auto;
  padding: 0 2rem;
}
.page-title {
  font-family: 'Lora', serif;
  font-size: 2rem;
  font-weight: 700;
  color: #f5edd6;
  margin-bottom: 0.25rem;
}
.page-sub {
  color: #a8c97f;
  font-size: 0.875rem;
}
.main-content {
  padding: 2rem 2rem 4rem;
}
.loading-wrap {
  display: flex;
  flex-direction: column;
  align-items: center;
  gap: 1rem;
  padding: 4rem;
  color: #6b7280;
}
.spinner {
  width: 32px;
  height: 32px;
  border: 3px solid #e8e0d0;
  border-top-color: #2d5016;
  border-radius: 50%;
  animation: spin 0.8s linear infinite;
}
.spinner-sm {
  width: 16px;
  height: 16px;
  border: 2px solid rgba(245, 237, 214, 0.3);
  border-top-color: #f5edd6;
  border-radius: 50%;
  animation: spin 0.7s linear infinite;
}
@keyframes spin {
  to {
    transform: rotate(360deg);
  }
}
.form-layout {
  display: flex;
  flex-direction: column;
  gap: 1.25rem;
}
.status-banner {
  display: flex;
  align-items: flex-start;
  gap: 0.85rem;
  padding: 1rem 1.25rem;
  border-radius: 12px;
  border: 1.5px solid;
}
.status-aktif {
  background: #f0fdf4;
  border-color: #bbf7d0;
}
.status-pending {
  background: #fffbeb;
  border-color: #fde68a;
}
.status-tolak {
  background: #fef2f2;
  border-color: #fecaca;
}
.status-blokir {
  background: #f9fafb;
  border-color: #d1d5db;
}
.status-icon-sm {
  font-size: 1.25rem;
  flex-shrink: 0;
}
.status-label {
  font-size: 0.7rem;
  color: #9ca3af;
  margin-bottom: 0.15rem;
  text-transform: uppercase;
  letter-spacing: 0.05em;
}
.status-val {
  font-size: 0.875rem;
  font-weight: 600;
  color: #374151;
}
.status-alasan {
  font-size: 0.78rem;
  color: #dc2626;
  margin-top: 0.25rem;
  font-style: italic;
}
.form-section {
  background: #fff;
  border: 1px solid #e8e0d0;
  border-radius: 16px;
  padding: 1.5rem;
}
.section-title {
  font-family: 'Lora', serif;
  font-size: 1.05rem;
  font-weight: 700;
  color: #1a2e0a;
  margin-bottom: 1.25rem;
}
.optional {
  font-size: 0.78rem;
  color: #9ca3af;
  font-weight: 400;
}
.foto-layout {
  display: flex;
  gap: 1.25rem;
  align-items: flex-start;
}
.foto-preview-wrap {
  position: relative;
  width: 100px;
  height: 100px;
  border-radius: 14px;
  overflow: hidden;
  flex-shrink: 0;
  border: 2px solid #e8e0d0;
}
.foto-preview-wrap:hover .foto-overlay {
  opacity: 1;
}
.foto-preview {
  width: 100%;
  height: 100%;
  object-fit: cover;
}
.foto-placeholder {
  width: 100%;
  height: 100%;
  background: #f0f7e8;
  display: flex;
  align-items: center;
  justify-content: center;
  font-size: 2.5rem;
}
.foto-overlay {
  position: absolute;
  inset: 0;
  background: rgba(0, 0, 0, 0.45);
  display: flex;
  align-items: center;
  justify-content: center;
  color: white;
  font-size: 0.75rem;
  font-weight: 600;
  opacity: 0;
  transition: opacity 0.2s;
}
.foto-info {
  flex: 1;
}
.foto-hint {
  font-size: 0.8rem;
  color: #6b7280;
  line-height: 1.5;
  margin-bottom: 0.75rem;
}
.btn-upload-foto {
  background: #f0f7e8;
  color: #2d5016;
  border: 1.5px solid #a8c97f;
  padding: 0.5rem 1rem;
  border-radius: 8px;
  font-size: 0.8rem;
  font-weight: 600;
  font-family: inherit;
  cursor: pointer;
  transition: background 0.2s;
}
.btn-upload-foto:hover {
  background: #e0f0cc;
}
.hidden {
  display: none;
}
.field {
  display: flex;
  flex-direction: column;
  gap: 0.4rem;
  margin-bottom: 1.1rem;
}
.field:last-child {
  margin-bottom: 0;
}
label {
  font-size: 0.82rem;
  font-weight: 600;
  color: #374151;
}
.field-hint {
  font-size: 0.72rem;
  color: #9ca3af;
}
input,
textarea {
  padding: 0.75rem 1rem;
  border: 1.5px solid #d1d5db;
  border-radius: 10px;
  font-size: 0.9rem;
  font-family: inherit;
  background: #fff;
  color: #111827;
  transition: border-color 0.2s;
  width: 100%;
}
input:focus,
textarea:focus {
  outline: none;
  border-color: #2d5016;
}
input:disabled,
textarea:disabled {
  background: #f9fafb;
  cursor: not-allowed;
}
textarea {
  resize: vertical;
}
.input-prefix-wrap {
  display: flex;
}
.prefix {
  background: #f3f4f6;
  border: 1.5px solid #d1d5db;
  border-right: none;
  border-radius: 10px 0 0 10px;
  padding: 0.75rem 0.75rem;
  font-size: 0.85rem;
  color: #6b7280;
  font-weight: 600;
  white-space: nowrap;
}
.input-prefix-wrap input {
  border-radius: 0 10px 10px 0;
}
.view-value {
  padding: 0.75rem 1rem;
  background: #f9fafb;
  border: 1.5px solid #e5e7eb;
  border-radius: 10px;
  font-size: 0.9rem;
  color: #111827;
  line-height: 1.5;
}
.multiline {
  white-space: pre-line;
}
.alert-error {
  background: #fef2f2;
  border: 1px solid #fecaca;
  color: #dc2626;
  padding: 0.75rem 1rem;
  border-radius: 8px;
  font-size: 0.875rem;
}
.alert-success {
  background: #f0fdf4;
  border: 1px solid #bbf7d0;
  color: #166534;
  padding: 0.75rem 1rem;
  border-radius: 8px;
  font-size: 0.875rem;
}
.form-actions {
  display: flex;
  gap: 0.75rem;
}
.btn-cancel {
  display: inline-flex;
  align-items: center;
  justify-content: center;
  background: #f3f4f6;
  color: #374151;
  border: 1.5px solid #d1d5db;
  padding: 0.85rem 1.5rem;
  border-radius: 10px;
  font-size: 0.9rem;
  font-weight: 600;
  font-family: inherit;
  cursor: pointer;
  text-decoration: none;
  transition: background 0.2s;
}
.btn-cancel:hover:not(:disabled) {
  background: #e5e7eb;
}
.btn-cancel:disabled {
  opacity: 0.5;
  cursor: not-allowed;
}
.btn-submit {
  flex: 1;
  display: flex;
  align-items: center;
  justify-content: center;
  gap: 0.5rem;
  background: #2d5016;
  color: #f5edd6;
  border: none;
  padding: 0.85rem;
  border-radius: 10px;
  font-size: 0.95rem;
  font-weight: 600;
  font-family: inherit;
  cursor: pointer;
  transition: background 0.2s;
}
.btn-submit:hover:not(:disabled) {
  background: #3a6b1e;
}
.btn-submit:disabled {
  opacity: 0.6;
  cursor: not-allowed;
}
@media (max-width: 768px) {
  .nav-links {
    display: none;
  }
  .hamburger {
    display: block;
  }
}
</style>
